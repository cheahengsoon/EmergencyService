...
namespace Emergency_Services
{
    ...
    public sealed partial class Hastane : Emergency_Services.Common.LayoutAwarePage
    {
        ...
        private async void btn_OlayGoster_Click(object sender, RoutedEventArgs e)
        {
            
            Service1Client dbService = new Service1Client();
            ObservableCollection<string> hastane = await dbService.HastaneGetirAsync();
            foreach (var item in hastane)
            {
                combo_Hastaneler.Items.Add(item);
            }
